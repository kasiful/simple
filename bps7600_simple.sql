...
--
-- Host: localhost:3306
-- Waktu pembuatan: 25 Jun 2020 pada 16.47
-- Versi server: 5.6.47
-- Versi PHP: 7.3.6
...
INSERT INTO `user` (`id`, `nama`, `username`, `password`, `leveluser_id`) VALUES
...
CREATE ALGORITHM=UNDEFINED DEFINER=`bps7600`@`localhost` SQL SECURITY DEFINER VIEW `list_kab`  AS  select `a`.`id` AS `prov_id`,`a`.`provinsi` AS `prov`,`b`.`id` AS `kab_id`,`b`.`kab` AS `kab` from (`master_prov` `a` join `master_kab` `b` on((`b`.`prov_id` = `a`.`id`))) ;
