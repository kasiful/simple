    ...
    public function record_bulanan($prov, $kab, $kantor_unit, $bulan, $tahun)
    {
        ...
        for ($a = 0; $a < count($pelabuhan); $a++) { // masing-masing pelabuhan ada pelayaran
            ...
            for ($i = 0; $i <= 5; $i++) { // pelra,nonpelra, perintis, nasional, luar negeri, khusus
                ...
                $sql = "select sum(r16) as sum_r16, r16s, r16n from " .
                    "simple_tbl_pdn_bongkar_barang where keygen in(select keygen from " .
                    "laporan_bulanan where prov=$prov " .
                    "and kab=$kab " .
                    "and MONTH(berangkat_tgl)=$bulan " .
                    "and YEAR(berangkat_tgl)=$tahun " .
                    "and pelabuhan_id = {$pelabuhan[$a]['id']} " .
                    "and jenis_pelayaran=$jenis_pelayaran_id " .
                    "and status=1 and approval=1) group by r16s, r16n";

                $temp_pdn_bongkar = DB::select($sql);
                foreach ($temp_pdn_bongkar as $x) {
                    $pelayaran[$i]['sum_pdn_bongkar'][] = (array) $x;
                }

                //--------------------------------------

                $sql = "select sum(r18) as sum_r18, r18s, r18n from " .
                    "simple_tbl_pdn_muat_barang where keygen in(select keygen from " .
                    "laporan_bulanan where prov=$prov " .
                    "and kab=$kab " .
                    "and MONTH(berangkat_tgl)=$bulan " .
                    "and YEAR(berangkat_tgl)=$tahun " .
                    "and pelabuhan_id = {$pelabuhan[$a]['id']} " .
                    "and jenis_pelayaran=$jenis_pelayaran_id " .
                    "and status=1 and approval=1) group by r18s, r18n";

                $temp_pdn_muat = DB::select($sql);
                foreach ($temp_pdn_muat as $x) {
                    $pelayaran[$i]['sum_pdn_muat'][] = (array) $x;
                }

                //--------------------------------------

                $sql = "select sum(r20) as sum_r20, r20s, r20n from " .
                    "simple_tbl_pln_bongkar_barang where keygen in(select keygen from " .
                    "laporan_bulanan where prov=$prov " .
                    "and kab=$kab " .
                    "and MONTH(berangkat_tgl)=$bulan " .
                    "and YEAR(berangkat_tgl)=$tahun " .
                    "and pelabuhan_id = {$pelabuhan[$a]['id']} " .
                    "and jenis_pelayaran=$jenis_pelayaran_id " .
                    "and status=1 and approval=1) group by r20s, r20n";

                $temp_pln_bongkar = DB::select($sql);
                foreach ($temp_pln_bongkar as $x) {
                    $pelayaran[$i]['sum_pln_bongkar'][] = (array) $x;
                }

                //--------------------------------------

                $sql = "select sum(r21) as sum_r21, r21s, r21n from " .
                    "simple_tbl_pln_muat_barang where keygen in(select keygen from " .
                    "laporan_bulanan where prov=$prov " .
                    "and kab=$kab " .
                    "and MONTH(berangkat_tgl)=$bulan " .
                    "and YEAR(berangkat_tgl)=$tahun " .
                    "and pelabuhan_id = {$pelabuhan[$a]['id']} " .
                    "and jenis_pelayaran=$jenis_pelayaran_id " .
                    "and status=1 and approval=1) group by r21s, r21n";

                $temp_pln_muat = DB::select($sql);
                foreach ($temp_pln_muat as $x) {
                    $pelayaran[$i]['sum_pln_muat'][] = (array) $x;
                }
